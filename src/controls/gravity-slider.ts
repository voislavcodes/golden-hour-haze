...
@customElement('ghz-gravity-slider')
export class GravitySlider extends BaseControl {
  static styles = [
  ...
  connectedCallback() {
    ...
    this.value = sceneStore.get().gravity;
    this._visible = uiStore.get().activeTool === 'form';
    this._updateHidden();
    this._unsubScene = sceneStore.select(
      (s) => s.gravity,
      (v) => { this.value = v; }
    );
    this._unsubUI = uiStore.select(
      (s) => s.activeTool,
      (tool) => {
        this._visible = tool === 'form';
        this._updateHidden();
      }
    );
  }
  ...
  render() {
    ...
    return html`
      <div class="slider-container glass">
        <span class="slider-label">grav</span>
        <div
          class="slider-track"
...
declare global {
  interface HTMLElementTagNameMap {
    'ghz-gravity-slider': GravitySlider;
  }
}
